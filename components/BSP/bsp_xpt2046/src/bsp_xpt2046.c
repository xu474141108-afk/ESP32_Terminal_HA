...
#define TOUCH_RAW_X_MIN 500
#define TOUCH_RAW_X_MAX 3700
#define TOUCH_RAW_Y_MIN 200
#define TOUCH_RAW_Y_MAX 3750

#define XPT2046_PD0_BIT     (0x01)  //01 to close PENIRQ, 0 to keep it open
#define XPT2046_PD1_BIT     (0x00)  //02 to enable VREF, 0 to disable it
...
enum xpt2046_registers
{
    ...
    // The following registers are not used by this driver but are defined here for completeness.
...
static esp_err_t xpt2046_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    XPT2046_LOCK(&tp->data.lock);
    #if CONFIG_XPT2046_CONVERT_XY
        tp->data.coords[0].x = y; // 12-bit ADC value to screen coordinate
        tp->data.coords[0].y = x; // 12-bit ADC value to screen coordinate
    #else
        tp->data.coords[0].x = x; // 12-bit ADC value to screen coordinate
        tp->data.coords[0].y = y; // 12-bit ADC value to screen coordinate
    #endif // CONFIG_XPT2046_CONVERT_ADC_TO_COORDS
