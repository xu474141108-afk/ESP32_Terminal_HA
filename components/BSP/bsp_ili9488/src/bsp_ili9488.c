    ...
    const void *color_data)
{
    ...
    {
        uint8_t *tmp = heap_caps_malloc(color_data_len * 3, MALLOC_CAP_DMA);
        if (!tmp) return ESP_ERR_NO_MEM;

        for (size_t i = 0; i < color_data_len; i++) {
            uint16_t color = src[i];
            
            // 按照 RGB 顺序 (Red -> Green -> Blue)
            tmp[i * 3 + 0] = ((color >> 8) & 0xF8) >> 1; 
            tmp[i * 3 + 1] = ((color >> 3) & 0xFC) >> 1; 
            tmp[i * 3 + 2] = ((color << 3) & 0xF8) >> 1;
        }

        // 4. 发送转换后的数据
        
        esp_lcd_panel_io_tx_color(io, LCD_CMD_RAMWR, tmp, color_data_len * 3);
        free(tmp);
    }
    else
    {
        uint16_t *tmp = heap_caps_malloc(color_data_len * 2, MALLOC_CAP_DMA);
        if (!tmp) return ESP_ERR_NO_MEM;
        for (size_t i = 0; i < color_data_len; i++) {
            tmp[i] = (src[i] << 8) | (src[i] >> 8);
        }
        // 16-bit color we can transmit as-is to the display.
        esp_lcd_panel_io_tx_color(io, LCD_CMD_RAMWR, tmp, color_data_len * 2);
        // 释放内存
        free(tmp);
    }
